  ...
  return (
    <div className="space-y-6 pt-6">
      ...
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        {cards.map((c) => (
          <Link key={c.title} href={c.href} className="rounded-xl bg-[#FFFFFF] p-5 hover:border-[#111111] border border-black/10">
      ...
      <section aria-label="Recent admin activity" className="rounded-xl bg-[#FFFFFF] p-5">
        ...
        {(audit.data ?? []).length === 0 ? (
          <p className="mt-2 text-sm text-[#6B7280]">No moderation actions recorded yet.</p>
        ) : (
          <ul className="mt-2 space-y-1 text-sm">
            {(audit.data ?? []).map((e: { action: string; target_type: string; target_id: string; created_at: string }, i: number) => (
              <li key={i} className="flex flex-wrap gap-2">
                ...
                <span className="text-[#6B7280]">{e.target_type} {e.target_id.slice(0, 8)} · {new Date(e.created_at).toLocaleString()}</span>
              </li>
            ))}
          </ul>
        )}
