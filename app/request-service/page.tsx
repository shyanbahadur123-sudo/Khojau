...
function RequestForm() {
    ...
    return (
      <div className="mx-auto max-w-xl pt-6">
        <div className="rounded-2xl bg-[#FFFDF8] p-8 text-center">
          ...
          <a href="/" className="mt-4 inline-block rounded-lg bg-[#0B7168] px-5 py-3 font-semibold text-white">Back home</a>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="mx-auto max-w-xl pt-6">
      <h1 className="text-2xl font-bold">Request a service</h1>
      <p className="mt-1 text-sm text-[#66706E]">Tell us what you need — {selected ? `${selected.business_name} will see it directly.` : "the Khojau team will manually match you with providers."}</p>
      <form
      ...
      >
        ...
        {status && !done && <p role="status" className="text-sm text-[#66706E]">{status}</p>}
      </form>
    </div>
  );
}
